...
interface SlideDetailClientPageProps {
  allImages: ImageWithHash[];
  fileId: string;
}

export default function SlideDetailClientPage({ allImages, fileId }: SlideDetailClientPageProps) {
  const router = useRouter();

  const { isLoggedIn, profile, isLiffReady, error: liffError, login, shareTargetPicker, isSharetargetPickerAvailable } = useLiff();
  const [images, setImages] = useState<ImageWithHash[]>(allImages);
  ...
  useEffect(() => {
    if (images.length > 0) {
      const index = images.findIndex(img => img.hashId === fileId);
      if (index === -1) {
